...
clusters = read.table(paste0(local,".dRep_fastANI_99.csv"), stringsAsFactors=F, head=T, sep=",") %>% mutate(cluster_99=paste0(secondary_cluster)) %>% select(genome, cluster_99) %>% left_join(cluster_95, .)
...
cl95_reps = clusters %>% arrange(cluster_95, -score) %>% distinct_at(.vars="cluster_95", .keep_all=T) %>% select(-cluster_99) %>% arrange(-score) %>% mutate(cluster_95_final=paste0(local,"_cluster95_",formatC(seq_along(cluster_95), width = 6, format = "d", flag = "0")))
cl99_reps = clusters %>% arrange(cluster_95, -score) %>% distinct_at(.vars="cluster_99", .keep_all=T) %>% select(-cluster_95) %>% arrange(-score) %>% mutate(cluster_99_final=paste0(local,"_cluster99_",formatC(seq_along(cluster_99), width = 6, format = "d", flag = "0")))

clusters = clusters %>% left_join(cl95_reps %>% select(cluster_95, cluster_95_final)) %>% left_join(cl99_reps %>% select(cluster_99, cluster_99_final))
