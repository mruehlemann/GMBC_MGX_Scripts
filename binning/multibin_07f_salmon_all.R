...
for(lv in unique(alltax$level)){
	print(lv)
	alltax %>% filter(level == lv) %>% reshape2::dcast(sample ~ tax, value.var="tpm", fill=0) %>% data.frame(row.names=1) %>% write.table(., paste0("allgroups/GMbC_final_table_",lv,".tsv"), sep="\t")
}
