...
clusters = read.table("GMbC.rep97.dRep_95.csv", stringsAsFactors=F, head=T, sep=",") %>% mutate(cluster_95=paste0(secondary_cluster)) %>% select(genome, cluster_95)


scores = read.table("GMbC.rep97.refined.out", head=T, stringsAsFactors=F) %>% tibble::rownames_to_column("genome") %>% mutate(score.gtdb_rel207_ar53 = ifelse(is.na(score.gtdb_rel207_ar53), 0, score.gtdb_rel207_ar53), score.gtdb_rel207_bac120 = ifelse(is.na(score.gtdb_rel207_bac120), 0, score.gtdb_rel207_ar53)) %>%
...
cl95_reps = clusters %>% arrange(cluster_95, -score) %>% distinct_at(.vars="cluster_95", .keep_all=T) %>% arrange(-score) %>% mutate(cluster_95_final=paste0("GMbC","_cluster95_",formatC(seq_along(cluster_95), width = 6, format = "d", flag = "0")))
...
all_clusters_w_rep = all_clusters %>% left_join(all_reps97 %>% select(cluster_97_final, genome_rep97=genome)) %>% left_join(clusters %>% select(genome_rep97=genome, GMbC_95_final = cluster_95_final))
